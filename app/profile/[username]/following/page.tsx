...
export default async function FollowingPage({ params }: Props) {
  ...
  // Get everyone this profile follows
  ...
  const following = ((follows ?? []).map(f => f.profiles).filter(Boolean) as unknown) as {
    id: string; username: string; full_name: string | null; avatar_url: string | null
  }[]

  // Get who the viewer already follows
      ...
      <main className="max-w-lg mx-auto">
        ...
        <div className="flex items-center gap-3 px-4 py-3 border-b border-zinc-100 dark:border-zinc-800 sticky top-14 md:top-0 bg-white dark:bg-zinc-950 z-10">
          ...
          <div>
            ...
            <p className="text-xs text-zinc-400">following {following.length}</p>
          </div>
        </div>

        {following.length === 0 ? (
          <div className="text-center py-20 text-zinc-400">
            <p className="font-semibold text-zinc-600 dark:text-zinc-300">Not following anyone yet</p>
            <p className="text-sm mt-1">
              <Link href="/explore" className="underline">Explore</Link> to find people to follow
            </p>
          </div>
        ) : (
